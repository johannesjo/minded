...
1. **Cutting a release.** `cd extension && npm run version patch|minor|major` → review diff → `git push --follow-tags` → approve both `production` env runs in the Actions UI → submit Chrome draft from dashboard → promote Play internal → production in Play Console.
