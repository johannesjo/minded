...
// How long the supportive line lingers before easing out to the resting sun —
// long enough to read a sentence, short enough not to demand attention. In the
// same register as the grounding flow's "Be proud!" praise.
const SUPPORTIVE_RESPONSE_MS = 3200;
...
export const Question: (props: {
  ...
  // Supportive off-ramp (see `Question.supportiveResponse`): a tapped chip names
  // a depleted state, so instead of proceeding into the choose-a-session flow we
  // show one supportive line and ease out to the resting sun via onSkip. The
  // existing content fades first, then the line fades in — never a hard cut.
  const [getSupportiveMsg, setSupportiveMsg] = createSignal<string | null>(
    null,
  );
  const [getIsContentExiting, setIsContentExiting] = createSignal(false);
  let supportiveSwapTimeout: number | undefined;
  let supportiveExitTimeout: number | undefined;
  onCleanup(() => {
    if (supportiveSwapTimeout) window.clearTimeout(supportiveSwapTimeout);
    if (supportiveExitTimeout) window.clearTimeout(supportiveExitTimeout);
  });

  const runSupportiveExit = (chip: string) => {
    props.onCancelCountdown();
    const answerTxt = question.prompt ? `${question.prompt} ${chip}` : chip;
    // Honour the save guard like any answer; UP8 sets isDontSaveAnswer, so the
    // named state leaves no record — naming it is the whole value.
    if (!question.isDontSaveAnswer) {
      void saveAnswer({
        questionCategoryId: question.categoryId,
        qid: question.id,
    ...
    supportiveSwapTimeout = window.setTimeout(() => {
      setSupportiveMsg(question.supportiveResponse ?? null);
    }, CHIPS_FADE_MS);
    supportiveExitTimeout = window.setTimeout(
      () => props.onSkip(),
      CHIPS_FADE_MS + SUPPORTIVE_RESPONSE_MS,
    );
  };
  ...
  const submitChip = (chip: string) => {
    if (question.supportiveResponse) {
      runSupportiveExit(chip);
      return;
    }
    props.onCancelCountdown();
    const answerTxt = question.prompt ? `${question.prompt} ${chip}` : chip;
  ...
  return (
    <div id="minded-6622-question-wrapper">
      <Show when={getSupportiveMsg()}>
        <div class="txtBig question-supportive" role="status">
          <span>{getSupportiveMsg()}</span>
        </div>
      </Show>
      ...
      <Show when={!getSupportiveMsg()}>
        <div
          ...
          classList={{ "is-exiting": getIsContentExiting() }}
        >
          <div
