...
export const BackgroundTransition: Component<BackgroundTransitionProps> = (props) => {
  ...
  onMount(() => {
    ...
    window.addEventListener('dragProgress', handleDragProgress as EventListener);
    window.addEventListener('startBackgroundAnimation', handleStartAnimation as EventListener);
    
    return () => {
      window.removeEventListener('dragProgress', handleDragProgress as EventListener);
      window.removeEventListener('startBackgroundAnimation', handleStartAnimation as EventListener);
      if (animationFrame) {
  ...
  const updateBackground = (progress: number) => {
    ...
    if (progress < 0) {
      ...
      topColor = interpolateColor(VISUAL_STATES.default.top, VISUAL_STATES.blueSky.top, factor);
      bottomColor = interpolateColor(VISUAL_STATES.default.bottom, VISUAL_STATES.blueSky.bottom, factor);
    } else if (progress > 0) {
      // Dragging down - interpolate from default to sunset
      const factor = progress;
      topColor = interpolateColor(VISUAL_STATES.default.top, VISUAL_STATES.sunset.top, factor);
      bottomColor = interpolateColor(VISUAL_STATES.default.bottom, VISUAL_STATES.sunset.bottom, factor);
    } else {
      // Default state
  ...
  const animateToCompletion = (direction: 'up' | 'down') => {
    ...
    const animate = () => {
      ...
      const currentProgress = startProgress + (targetProgress - startProgress) * easeInOut;
      setProgress(currentProgress);
      updateBackground(currentProgress);
